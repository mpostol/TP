...
namespace LinqToSqlLib.Tests
{
  ...
  public class PersonServiceTests
  {
    ...
    public void Init()
    {
      m_Service = new PersonService(m_ConnectionString);
    }
    [TestCleanup]
    public void CleanUp()
    {
    ...
    {
      IEnumerable<Person> initialData = m_Service.GetAllPersons();
    ...
    public void FilterPersonsByLastName_FilterTwoSmith_OutOfFivePersons()
    {
      CreatePersons(3, "Person", startAge: 20);
      CreatePersons(2, "Smith", startAge: 11);
      IEnumerable<Person> data = m_Service.FilterPersonsByLastName("Smith");
      PrintPersons(data);
    ...
    public void FilterPersonsByMinAge_FindTwoPersonsInAge25OrMore()
    {
      ...
      IEnumerable<Person> filtered = m_Service.FilterPersonsByMinAge(25);
    ...
    public void ChangeAgeThenFilterPersonsByMinAge_FindFourPersonsInAge25OrMore()
    {
      ...
      IEnumerable<Person> filtered = m_Service.ChangeAgeThenFilterPersonsByMinAge(change: 13, minAge: 25);
      PrintPersons(filtered);
    ...
    public void ChangeAgeThenFilterPersonsByMinAge_MakeOlderThenYounger_OriginalAgeShouldBeRestored()
    {
      ...
      IEnumerable<Person> persons = m_Service.GetAllPersons();
      int[] originalAge = new int[persons.Count()];
      for (int i = 0; i < persons.Count(); i++)
        originalAge[i] = persons.ElementAt(i).Age;
      IEnumerable<Person> oldPersons = m_Service.ChangeAgeThenFilterPersonsByMinAge(change: 100, minAge: 120);
      Assert.AreEqual(persons.Count(), oldPersons.Count());
      foreach (Person p in oldPersons)
        Assert.IsTrue(p.Age >= 120);
      IEnumerable<Person> youngPersons = m_Service.ChangeAgeThenFilterPersonsByMinAge(change: -100, minAge: 20).ToArray();
    ...
    private readonly string m_ConnectionString = global::LinqToSqlLibTests.Properties.Settings.Default.UnitTestDataConnectionString;
    ...
    private void CreatePersons(int number, string lastName, int startAge, int ageStep = 5)
    {
      for (int i = 0; i < number; i++)
      {
        ...
        m_Service.AddPerson(new Person() { FirstName = "Test " + N, LastName = lastName, Age = age });
      }
    }
...
#if PRINT_PERSONS
      foreach (Person p in data)
        Debug.WriteLine("Person(PersonId={0}) : {1} {2}, age {3}", p.PersonId, p.FirstName, p.LastName, p.Age);
#endif
    }
    #endregion
